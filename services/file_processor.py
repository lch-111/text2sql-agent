...
class PDFProcessor:
    ...
    使用 pdfplumber 优先提取表格和文本，PyPDF2 作为后备。
    """
    ...
    def _check_dependencies(self):
        ...
        try:
            ...
            logger.warning("[PDF] pdfplumber 未安装，表格提取降级")
            self._pdfplumber_available = False
    ...
    def is_available(self) -> bool:
        """检查 PDF 解析功能是否可用"""
        return self._pdfplumber_available or self._pypdf2_available

    def parse(self, file_bytes: bytes, filename: str) -> FileContent:
        """
        解析 PDF 文件，优先用 pdfplumber 提取表格和文本。
        ...
        返回:
            FileContent 对象
        """
        content = FileContent(filename=filename, file_type="pdf", row_count=0, pages=[], sheets={})
        full_text_parts = []

        # 优先使用 pdfplumber（表格提取能力强）
            ...
            try:
                ...
                with pdfplumber.open(io.BytesIO(file_bytes)) as pdf:
                    for i, page in enumerate(pdf.pages, 1):
                        ...
                        if raw_tables:
                            for t_idx, table in enumerate(raw_tables):
                                if table and len(table) > 1:
                                    ...
                                    sheet_name = f"page{i}_table{t_idx+1}"
                        ...
                        full_text_parts.append(f"【第{i}页】\n{text}")
                        if text.strip():
                            content.row_count += len(text.split("\n"))

                    logger.info(f"[PDF] pdfplumber 解析完成: {filename} | {len(pdf.pages)} 页 | {len(content.sheets)} 个表格")
                    full_text = "\n".join(full_text_parts)
                    content.summary = self._generate_summary(full_text, content)
                    return content
            except Exception as e:
                logger.warning(f"[PDF] pdfplumber 解析失败，降级到 PyPDF2: {e}")
        ...
        if self._pypdf2_available:
            try:
                ...
                for page_num, page in enumerate(reader.pages, 1):
                    text = page.extract_text() or ""
                    content.pages.append({"page_num": page_num, "text": text, "tables": []})
                ...
                full_text = "\n".join(full_text_parts)
                content.summary = self._generate_summary(full_text, content)
                logger.info(f"[PDF] PyPDF2 解析完成: {filename} | {len(reader.pages)} 页")
                return content
            except Exception as e:
                return FileContent(filename=filename, file_type="pdf", error=f"PDF 解析失败: {e}")

        return FileContent(filename=filename, file_type="pdf", error="pdfplumber 和 PyPDF2 均未安装")
            return content
